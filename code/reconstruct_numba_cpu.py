...
# Target paths
banana_path = PROJECT_ROOT / "results" / "csv files" / f"Numba_CPU_stats_banana_{platform_env}.csv"
apple_path = PROJECT_ROOT / "results" / "csv files" / f"Numba_CPU_stats_apple_{platform_env}.csv"
vase_path = PROJECT_ROOT / "results" / "csv files" / f"Numba_CPU_stats_vase_{platform_env}.csv"
fig_path = PROJECT_ROOT / "results" / "pdf fig files" / f"numba_mt_cpu_{platform_env}.pdf"
...
print("\n" + "=" * 75)
print("🚀 Surface Reconstruction Benchmark (Numba MultiThread | CPU)")
print("=" * 75)
        ...
        for ds in ['banana', 'apple', 'vase']:
            for i in range(-WARMUP, R):
                ...
                if dt == np.float64:
                    ...
                    gR, gz, gRB, gRC, fb, fc = surf_tangent_numba_f64(R_mat, N, tot_pts, Z, Null_Hts, B_Point, C_Point,
                                                                      B, T, bt, ct)
                    t4 = time.perf_counter()
                    FR, Fz = surf_pts_numba_f64(R_mat, N, tot_pts, Z, B_Point, C_Point, B, T, gRB, gRC, fb, fc, gR, gz,
                                                bt, ct, n2)
                    ...
                    t3 = time.perf_counter()
                    gR, gz, gRB, gRC, fb, fc = surf_tangent_numba_f32(R_mat, N, tot_pts, Z, Null_Hts, B_Point, C_Point,
                                                                      B, T, bt, ct)
                    t4 = time.perf_counter()
                    FR, Fz = surf_pts_numba_f32(R_mat, N, tot_pts, Z, B_Point, C_Point, B, T, gRB, gRC, fb, fc, gR, gz,
                                                bt, ct, n2)
                    t5 = time.perf_counter()
...
for ds in ['banana', 'apple', 'vase']:
    ...
    if ds == 'banana':
        FR_banana, Fz_banana = surf_pts_numba_f64(R, N, tot_pts, Z, B_Point, C_Point, B, T, gRB, gRC, fb, fc, gR, gz,
                                                  bt, ct, n2)
    elif ds == 'apple':
        FR_apple, Fz_apple = surf_pts_numba_f64(R, N, tot_pts, Z, B_Point, C_Point, B, T, gRB, gRC, fb, fc, gR, gz, bt,
                                                ct, n2)
    else:
        FR_vase, Fz_vase = surf_pts_numba_f64(R, N, tot_pts, Z, B_Point, C_Point, B, T, gRB, gRC, fb, fc, gR, gz, bt,
                                              ct, n2)
...
with PdfPages(fig_path) as pdf:
    ...
    fig.suptitle(
        ...
        "Interface: Numba (CPU), loop based implementation",
        fontsize=11,
        y=0.02
    )
