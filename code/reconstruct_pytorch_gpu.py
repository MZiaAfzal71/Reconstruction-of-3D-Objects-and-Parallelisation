...
# Target paths
banana_path = PROJECT_ROOT / "results" / "csv files" / f"TensorV_GPU_stats_banana_{platform_env}.csv"
apple_path = PROJECT_ROOT / "results" / "csv files" / f"TensorV_GPU_stats_apple_{platform_env}.csv"
vase_path = PROJECT_ROOT / "results" / "csv files" / f"TensorV_GPU_stats_vase_{platform_env}.csv"
fig_path = PROJECT_ROOT / "results" / "pdf fig files" / f"pytorch_gpu_{platform_env}.pdf"
...
def _peak_mem_mb_start(device):
    if device.type == 'cuda':
        torch.cuda.reset_peak_memory_stats(device)
        return None
    else:
        # baseline RSS immediately before this run; ru_maxrss is a
        # monotonic high-water mark for the whole process, so we record
        # the "before" value and report the delta after the run.
        return resource.getrusage(resource.RUSAGE_SELF).ru_maxrss  # KB on Linux


def _peak_mem_mb_end(device, baseline):
    if device.type == 'cuda':
        return torch.cuda.max_memory_allocated(device) / (1024 ** 2)
    else:
        after = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
        return max(after - baseline, 0) / 1024.0  # KB -> MB
        ...
        for dt in dtypes:
            for ds in objects:
                for i in range(-warmup, R):
                    ...
                    baseline_mem = _peak_mem_mb_start(device)
                    ...
                    I = [pt.to(device) for pt in I] if isinstance(I, list) else I.to(device)
                    Z = Z.to(device)
                    Null_Hts = Null_Hts.to(device)
                    _sync(device)
                    ...
                    peak_mem_mb = _peak_mem_mb_end(device, baseline_mem)
                    ...
                    if not is_warmup:
                        results[ds].loc[len(results[ds])] = [
                            ...
                            t_kernel_surf_pts, t_d2h_transfer, t_total, peak_mem_mb]
...
print("\n" + "=" * 75)
print("🚀 Surface Reconstruction Benchmark (PyTorch Vectorized | GPU)")
print("=" * 75)
...
with PdfPages(fig_path) as pdf:
    ...
    fig.suptitle(
        ...
        "Interface: PyTorch (GPU), vectorized implementation",
        fontsize=11,
        y=0.02
    )
